...
def create_output_structure(output_path, project_name, project_timestamp):
    ...
    if not os.path.exists(
        os.path.join(output_path, project_name + "_" + project_timestamp)
    ):
        os.mkdir(os.path.join(output_path, project_name + "_" + project_timestamp))
    if not os.path.exists(
        os.path.join(output_path, project_name + "_" + project_timestamp, "wavs")
    ):
        os.mkdir(
            os.path.join(output_path, project_name + "_" + project_timestamp, "wavs")
        )
...
def segment_audio_file(
    file_path, model, seg_onset, seg_offset, seg_min_duration, seg_min_duration_off
):
...
def segment_file_by_diargroup(file_path, output_path, groups, gidx=-1):
    ...
    for g in groups:
        ...
        # print(start, end)
        gidx += 1
        file = audio[start:end].export(
            os.path.join(output_path, str(gidx) + ".wav"), format="wav"
        )
        # print(file)
    return gidx
