...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.getcwd() + os.sep + "output",
    )
    parser.add_argument("-project_name", help="project_name", type=str, default="none")
    parser.add_argument(
        "-whisper_model",
        help="Which whisper model to use, HF repo address",
        default="openai/whisper-large-v2",
    )
    parser.add_argument(
        "-device", help="Which device to use, cpu or cuda", default="cuda"
    )
    parser.add_argument(
        "-mode",
        help="Automatic diarization and segmentation",
        default="segment and transcribe",
        type=str,
        choices=["auto", "segment and transcribe", "diarize", "transcribe"],
    )
    parser.add_argument(
        "-diaization_model",
        help="Which diarization model to use, HF repo address",
        default="pyannote/speaker-diarization",
    )
    parser.add_argument(
        "-segmentation_model",
        help="Which segmentation model to use, HF repo address",
        default="pyannote/segmentation",
    )
    parser.add_argument(
        "-seg_onset",
        help="onset activation threshold, influences the segment detection",
        default=0.6,
        type=float,
    )
    parser.add_argument(
        "-seg_offset",
        help="offset activation threshold, influences the segment detection",
        default=0.4,
        type=float,
    )
    parser.add_argument(
        "-seg_min_duration",
        help="minimum duration of a segment, remove speech regions shorter than that many seconds.",
        default=2.0,
        type=float,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-valid_ratio", help="Ratio of validation data", default=0.2, type=float
    )
    parser.add_argument(
        "-ignore-cofnig",
        help="Ignore the config, specifiy your own setting sin CLI",
        action="store_true",
    )
    ...
    cprint(figlet_format("OZEN", font="starwars"), "yellow", "on_black", attrs=["bold"])
    ...
    output_path = create_output_structure(
        args.output_path, args.project_name, timestamp
    )
    ...
    if os.path.isfile(file_path):
        ...
        if file_ext in [".mp3", ".m4a", ".flac", ".ogg", ".wav"]:
            if file_ext != ".wav":
                print(
                    colorama.Fore.GREEN + "Converting to WAV..." + colorama.Fore.RESET
                )
            ...
            if args.mode == "auto":
                print(
                    colorama.Fore.GREEN
                    + "Loading Diraization Model..."
                    + colorama.Fore.RESET
                )
                pipe = load_pyannote_audio_pipeline(
                    args.diaization_model, args.hf_token
                )
                ...
                print(
                    colorama.Fore.GREEN
                    + "Loading Transcribing Model..."
                    + colorama.Fore.RESET
                )
                transcribe_pipe = init_transcribe_pipeline(
                ...
                for wav in range(0, wavs):
                    ...
                    res = transcribe_audio(
                        os.path.join(os.getcwd(), wav + ".wav"), transcribe_pipe
                    )
                    pb.update(1)
            if args.mode == "segment and transcribe":
                ...
                segments = segment_audio_file(
                    ...
                    args.seg_onset,
                    args.seg_offset,
                    args.seg_min_duration,
                    args.seg_min_duration_off,
                )
                ...
                transcribe_pipe = init_transcribe_pipeline(
                    args.whisper_model, 0 if args.device == "cuda" else -1
                )
                print(colorama.Fore.GREEN + "Transcribing..." + colorama.Fore.RESET)
                ...
                for wav in range(0, amount_to_train):
                    wav = str(wav)
                    res = transcribe_audio(
                        os.path.join(wavs_path, wav + ".wav"), transcribe_pipe
                    )[1:]
                    # add to train file
        ...
        for filename in os.listdir(file_path):
            ...
            if file_ext in [".mp3", ".m4a", ".flac", ".ogg", ".wav"]:
                ...
                if args.mode == "auto":
                    ...
                    pipe = load_pyannote_audio_model(
                        args.diaization_model, args.hf_token
                    )
                    print(colorama.Fore.GREEN + "Diarizing..." + colorama.Fore.RESET)
                    diarization = diarize_audio_file(file_path, pipe)
                    ...
                    for wav in range(0, amount_to_train):
                        ...
                        # add to train file
                        add_to_textfile(
                            train_file, "wavs/" + wav + ".wav|" + res + "\n"
                        )
                        pb.update(1)
                    for wav in range(
                        amount_to_train, amount_to_train + amount_to_valid
                    ):
                        gidx += 1
                        wav = str(wav)
                        res = transcribe_audio(
                ...
                if args.mode == "segment and transcribe":
                    ...
                    pipe = load_pyannote_audio_model(
                        args.segmentation_model, args.hf_token
                    )
                    segments = segment_audio_file(
                        file_path,
                        pipe,
                    ...
                    for wav in range(0, amount_to_train):
                        ...
                        wav = str(wav)
                        res = transcribe_audio(
                            os.path.join(wavs_path, str(gidx) + ".wav"), transcribe_pipe
                        )[1:]
                        # add to train file
                        add_to_textfile(
                            train_file, "wavs/" + str(gidx) + ".wav|" + res + "\n"
                        )
                        pb.update(1)
                    for wav in range(
                        amount_to_train, amount_to_train + amount_to_valid
                        ...
                        # add to valid file
                        add_to_textfile(
                            valid_file, "wavs/" + str(gidx) + ".wav|" + res + "\n"
                        )
                        pb.update(1)
